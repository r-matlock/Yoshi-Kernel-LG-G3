...
static int cpufreq_governor_darkness(struct cpufreq_policy *policy, unsigned int event);
...
static void update_sampling_rate(unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (time_before(next_sampling, appointed_at)) {
			...
			mutex_lock(&darkness_cpuinfo->timer_mutex);
			queue_delayed_work_on(darkness_cpuinfo->cpu, system_wq,
				&darkness_cpuinfo->work, usecs_to_jiffies(new_rate));
		}
		mutex_unlock(&darkness_cpuinfo->timer_mutex);
	}
}
...
static void darkness_check_cpu(struct cpufreq_darkness_cpuinfo *this_darkness_cpuinfo)
{
	...
	if (wall_time >= idle_time) {
		cur_load = wall_time > idle_time ? (100 * (wall_time - idle_time)) / wall_time : 1;
